...
public final class EasyBasicColumnRange extends EasyGraph {
	...
	protected void iniciaGrafico() {
		...
		super.chart.put("type" , "columnrange");
		super.chart.put("inverted" , true);
		...
		s1.addValor(Math.random() * 100, Math.random() * 100)
			.addValor(Math.random()* 100, Math.random() * 100)
			.addValor(Math.random()* 100, Math.random() * 100)
			.addValor(Math.random()* 100, Math.random() * 100);
