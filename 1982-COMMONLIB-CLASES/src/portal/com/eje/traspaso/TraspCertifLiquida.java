...
public class TraspCertifLiquida extends Proceso
{

    public TraspCertifLiquida(Connection conOrigen, Connection conDestino, String fecha_desde)
    {
    ...
    public TraspCertifLiquida(Connection conOrigen, Connection conDestino, String agno, String mes, String periDest)
    {
        super(conOrigen, conDestino);
        this.agno = agno;
        this.mes = mes;
    ...
    public boolean Run(int empresa, int periodo)
    {
        ...
        OutMessage.OutMessagePrint("\nActualizando Data Liquidacion de Remuneraciones");
        ...
        if(consulAuxx.next())
        {
            ...
            while(consulOri.next()) 
            {
                ...
                if(mes.length() == 1)
                    mes = "0" + mes;
                String delHistory = "delete from eje_ges_certif_histo_liquidacion_cabecera where  periodo = " + dato.objectToDato(consulOri.getString("ANO_PERIODO") + mes) + " AND empresa = " + dato.objectToDatoComillas(consulOri.getString("COD_EMPRESA")) + " AND rut = " + rut;
                consulDest.insert(delHistory);
                sqlDest = "INSERT INTO eje_ges_certif_histo_liquidacion_cabecera (periodo,empresa, unidad, rut, causa_pago, fec_pago, id_forma_pago,forma_pago, cuenta, imp_tribut, imp_no_tribut, no_imp_tribut,no_imp_no_tribut, reliq_rentas, tot_haberes, tot_desctos,liquido, tope_imp, val_uf, dctos_varios, dctos_legales,dctos_impagos,banco, wp_cod_empresa, wp_cod_planta, wp_tot_imponible, wp_afecto_imponible, wp_ndias_trab) VALUES (" + dato.objectToDato(consulOri.getString("ANO_PERIODO") + mes) + "," + dato.objectToDatoComillas(consulOri.getString("COD_EMPRESA")) + "," + dato.objectToDatoComillas(consulOri.getString("COD_UNIDAD_ADMINIS")) + ", " + rut + ", null" + ", null" + ", null" + ", null" + ", " + dato.objectToDatoComillas(nro_cta_cte) + "," + dato.objectToDato(consulOri.getString("VALOR_IMPONIBLE")) + ", null" + ", null" + "," + dato.objectToDato(consulOri.getString("HAB_NO_IMP_NO_TRIB")) + ", null" + "," + dato.objectToDato(consulOri.getString("VALOR_TOTAL_HABERE")) + "," + dato.objectToDato(consulOri.getString("VAL_TOTAL_DESCTOS")) + "," + dato.objectToDato(consulOri.getString("VAL_LIQUIDO_PAGO")) + "," + dato.objectToDato(consulOri.getString("TOT_IMPONIBLE")) + "," + dato.objectToDato(uf_mes) + "," + dato.objectToDato(consulOri.getString("TOT_DCTOS_NO_LEGAL")) + "," + dato.objectToDato(consulOri.getString("TOT_DCTOS_LEGALES")) + ", null" + "," + dato.objectToDatoComillas(n_banco) + "," + dato.objectToDato(consulOri.getString("COD_EMPRESA")) + "," + dato.objectToDato(consulOri.getString("COD_PLANTA")) + "," + dato.objectToDato(tot_imponible) + "," + dato.objectToDato(afecto_imponible) + "," + dato.objectToDato(ndias_trab) + ")";
                ...
                periodoHoy = dato.objectToDato(consulOri.getString("ANO_PERIODO") + mes);
                sqlDest = "update eje_ges_trabajador set tot_haberes = b.tot_haberes,tot_descuento=b.tot_desctos,liquido=b.liquido FROM eje_ges_trabajador a,eje_ges_certif_histo_liquidacion_cabecera b WHERE b.periodo = " + dato.objectToDato(periodoHoy) + " AND a.rut = b.rut AND a.wp_cod_empresa = b.wp_cod_empresa";
            }
            consulOri.close();
            consulAux.close();
            consulAux2.close();
            ...
            OutMessage.OutMessagePrint("Fin de Traspaso Data Liquidacion de Remuneraciones");
        }
        consulAuxx.close();
        return true;
    }
