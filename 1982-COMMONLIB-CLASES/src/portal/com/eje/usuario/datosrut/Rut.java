package portal.com.eje.usuario.datosrut;
...
public class Rut
{
    ...
    public Rut(Connection conn, String rut)
    {
        ...
        if(conn != null)
        {
            ...
            sql = "SELECT rut, digito_ver, id_unidad, id_cargo, empresa, nombre=ltrim(rtrim(nombre)), cargo=ltrim(rtrim(cargo)), sindicato, afp, isapre, division, ubic_fisica, area, rut_supd, dig_supd, nom_supd, cargo_supd, unidad=ltrim(rtrim(unidad)), fecha_nacim, fecha_ingreso, estado_civil, domicilio=ltrim(rtrim(domicilio)), comuna=ltrim(rtrim(comuna)), ciudad=ltrim(rtrim(ciudad)), telefono, celular, anexo, ape_paterno=ltrim(rtrim(ape_paterno)), ape_materno=ltrim(rtrim(ape_materno)), nombres=ltrim(rtrim(nombres)), e_mail, mail, fecha_cargo, antig_empresa, antig_cargo, pais, nacionalidad, sexo, foto, grupo_sangre, num_cargas, sueldo, empresa_descrip=ltrim(rtrim(empresa_descrip)) FROM view_rut_all WHERE (rut = " + rut + ")";
            consul.exec(sql);
            if(consul.next())
            {
                valido = true;
                ...
                sql = "SELECT anexo, rut, empresa, unidad FROM eje_ges_trabajadores_anexos WHERE (empresa = '" + Empresa + "') " + "AND (rut = " + rut + ") " + "AND (unidad = '" + Id_Unidad + "')";
                ...
                sql = "select e_mail,mail,anexo,unidad,foto from view_rut_all WHERE (rut = " + consul.getString("rut_supd") + ")";
                consul2.exec(sql);
                if(consul2.next())
                {
    ...
    public SimpleList getPrestamos(Connection conn, String rut)
    {
        ...
        if(conn != null)
        {
            ...
            sql = "SELECT movimiento, deuda, fecha, monto_inicial, total_cuotas, cuotas_pendientes, valor_cuota, saldo FROM eje_ges_cuentas_corrientes  WHERE (rut = " + rut + ")" + " ORDER BY fecha";
            deudas.exec(sql);
            if(!deudas.next())
            {
    ...
    public SimpleList getHijos(Connection conn, String rut)
    {
        ...
        if(conn != null)
        {
            ...
            sql = "SELECT rut, nombre,DATEDIFF(month, fecha_nacim, GETDATE()) / 12 AS edad,rut_carga,fecha_nacim,sexo  FROM eje_ges_grupo_familiar  WHERE (rut =" + rut + ")" + " and parentesco = 'Hijo(a)' ORDER BY fecha_nacim";
            hijos.exec(sql);
            int x = 0;
            SimpleHash simplehash1;
    ...
    public SimpleList getBeneficios(Connection conn, String empresa)
    {
        ...
        if(conn != null)
        {
            ...
            sql = "SELECT descripcion, val_ant, val_actual  FROM view_beneficios  WHERE (empresa ='" + empresa + "')";
            beneficios.exec(sql);
            int x = 0;
            SimpleHash simplehash1;
