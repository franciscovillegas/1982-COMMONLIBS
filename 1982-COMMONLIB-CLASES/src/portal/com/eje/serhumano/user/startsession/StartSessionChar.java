...
public class StartSessionChar extends AbsClaseWebInsegura {

    public StartSessionChar(IOClaseWeb ioClaseWeb) {
		super(ioClaseWeb);
    ...
    private ConsultaData getId(String usuario)
    {
      
        String sql = "select * from eje_ges_usuario_rut where usuario = ?";
	...
	public void doPost() throws Exception {
        ...
        String usuario = super.getIoClaseWeb().getParamString("usuario", null);
        ConsultaData id = this.getId(usuario);
        
        String pRut = new String();
        if(id.next()){
        	Integer rutInt = id.getInt("id"); 
        	pRut = rutInt.toString();
        }
        else{
        	pRut = "0";
        }
        ...
        if(user.esValido() && esUserVigente && !userBloqueado && !userBloqueadoNoUso) {
            ...
            if(envioAviso) {
            	...
            	loginGet(super.getIoClaseWeb().getReq(),super.getIoClaseWeb().getResp(),"login/loginadmin.html"," El Usuario y la Clave secreta ingresada no coinciden.");
            }
