...
public class StartSessinWF_userEmail_GetParams extends AbsClaseWebInsegura {
	private String pHtm;
	private String pHtmAdmin;
	
    public StartSessinWF_userEmail_GetParams(IOClaseWeb ioClaseWeb) {
		super(ioClaseWeb);
		
		pHtm = "macc/login/loginWF.html";
		pHtmAdmin = "macc/login/loginWFAdmin.html";
	}
    ...
    private void loginGet(HttpServletRequest req, HttpServletResponse resp,String pHtm, String msgRespuesta) 
    	...
    	try {
    		if("application/json".equals(super.getIoClaseWeb().getParamString("return_type", ""))) {
          		super.getIoClaseWeb().retSenchaJson(msgRespuesta, false);
          	}
          	else {
	...
	public void doPost() throws Exception {
        ...
        pHtm = "../servlet/Tool?htm=macc/home_qsm.html";
        pHtm = pHtm + "&hide_banner="+super.getIoClaseWeb().getParamString("hide_banner","false");
        pHtm = pHtm + "&hide_close="+super.getIoClaseWeb().getParamString("hide_close","false");
        pHtm = pHtm + "&prms="+super.getIoClaseWeb().getParamString("prms","{}");
        ...
        if(rut != null) {        	
        	...
        	if("true".equals(conTerceraClave)) {
	        	...
	        	if(!pageOut) {
	        		loginGet(super.getIoClaseWeb().getReq(),super.getIoClaseWeb().getResp(),pHtmAdmin,"C&oacute;digo \"Captcha\" inv&aacute;lido.");
	        		return;
	        	}
        	}
        ...
        if(user.esValido() && esUserVigente && !userBloqueado && !userBloqueadoNoUso) {
            ...
            return;
	            ...
	            if(envioAviso) {
	                ...
	                loginGet(super.getIoClaseWeb().getReq(),super.getIoClaseWeb().getResp(), pHtmAdmin ,proper.getString("clave.mensajebloqueo") + " " + tipobloqueo);
	            }
	            else if (usermgr.userBloqueado(pRut)) {
	            	String tipobloqueo = userBloqueadoNoUso ? proper.getString("clave.mensajebloqueoEmail2") : proper.getString("clave.mensajebloqueoEmail");
	            	loginGet(super.getIoClaseWeb().getReq(),super.getIoClaseWeb().getResp(), pHtmAdmin ,proper.getString("clave.mensajebloqueo2") + " " + tipobloqueo);
	            }
	            else if (user.getPassWord() != null && !user.getPassWord().equals(user.getPasswordBrowser())) {
	            	loginGet(super.getIoClaseWeb().getReq(),super.getIoClaseWeb().getResp(), pHtmAdmin," El Rut y la Clave secreta ingresada no coinciden.");
	            }
            
	            loginGet(super.getIoClaseWeb().getReq(),super.getIoClaseWeb().getResp(), pHtmAdmin," ");
	...
	public void doGet() throws Exception {
		if(super.getIoClaseWeb().getParamString("rut", null) != null ) {
			doPost();
		}
		else {
			loginGet(super.getIoClaseWeb().getReq(),super.getIoClaseWeb().getResp(),pHtm,"");	
		}
