...
public class MantenerUsuario extends MyHttpServlet
{
        ...
        throws IOException, ServletException
    {
        ...
        } else
        {
            ...
            String paramAccion = queAccion(req, conexion);
            String AccionOriginal = paramAccion;
            String paramClave = req.getParameter("clave");
            String acceso[] = req.getParameterValues("acceso");
            ...
            for(int j = 0; j < acceso.length; j++)
                try
                {
                    large += req.getParameterValues("acc_" + acceso[j]).length;
                }
                catch(Exception exception) { }

            String accesosSelec[] = (String[])Array.newInstance(java.lang.String.class, large);
            for(int j = 0; j < acceso.length; j++)
            {
                String tmp[] = req.getParameterValues("acc_" + acceso[j]);
                if(tmp != null)
                {
                    for(int k = 0; k < tmp.length; k++)
                        accesosSelec[k + retro] = tmp[k];
            ...
            if(paramAccion.equals("C"))
            {
                ...
                if(!es_certif && !es_anexo && largo_accesosSelec >= 1)
                {
                    ...
                    if(seguir = usermanager.crearUsuarioDF(paramRut, paramClave))
                        paramAccion = "M";
                    else
                        strMensajeFinal = "Usuario no creado...";
                } else
                {
                    ...
                    if(largo_accesosSelec > 1)
                    {
                        ...
                        if(!jerar && (seguir = usermanager.crearUsuarioDF(paramRut, paramClave)))
                            paramAccion = "M";
                        if(es_certif)
                            if(seguir = usermanager.crearUsuarioCertif(paramRut, paramClave))
                                paramAccion = "M";
                ...
                if(!es_certif && !es_anexo && largo_accesosSelec >= 1 && !paramAccion.equals("M"))
                {
            ...
            {
                if(!usermanager.existeUsuarioDF(req.getParameter("rut")) && (seguir = usermanager.crearUsuarioDF(paramRut, paramClave)))
                    paramAccion = "M";
                if(solo_certif)
                {
                    OutMessage.OutMessagePrint("%%%%%%%--> Mantener---Usuario solo de certificados");
                    usermanager.quitarUsuarioParam(paramRut, "eje_ges_usuario_certif");
                    if(!(seguir = usermanager.crearUsuarioCertif(paramRut, paramClave)))
                        strMensajeFinal = "Acci\363n Realizada con exito...";
                } else
                if(es_certif)
                ...
                if(solo_anexos)
                {
                    ...
                    usermanager.quitarUsuarioParam(paramRut, "eje_ges_usuario_anexos");
                    if(!(seguir = usermanager.crearUsuarioAnexos(paramRut, paramClave)))
                        strMensajeFinal = "Acci\363n Realizada con exito...";
                } else
                if(es_anexo)
            ...
            generaDatos(req, resp, paramAccion, strMensajeFinal, conexion);
        }
        usermanager.close();
        ...
        throws IOException, ServletException
    {
        ...
        {
            generaDatos(req, resp, paramAccion, null, Conexion);
        }
        usermanager.close();
        connMgr.freeConnection("portal", Conexion);
    }

    public void generaDatos(HttpServletRequest req, HttpServletResponse resp, String paramAccion, String mensajeFinal, Connection conexion)
        throws IOException, ServletException
    {
        ...
        if(Usuario.rescatarUsuario(req).esValido())
        {
            ...
            String tabla = "";
            String paramRut = req.getParameter("rut");
            if(ExisteEnTabla(paramRut, "eje_ges_usuario", conexion))
                tabla = "eje_ges_usuario";
            else
            if(ExisteEnTabla(paramRut, "eje_ges_usuario_certif", conexion))
                tabla = "eje_ges_usuario_certif";
            else
            if(ExisteEnTabla(paramRut, "eje_ges_usuario_anexos", conexion))
                tabla = "eje_ges_usuario_anexos";
            SimpleHash modelRoot = new SimpleHash();
            Consulta consul = new Consulta(conexion);
            String sql = String.valueOf(String.valueOf((new StringBuilder("SELECT nombre,cargo,digito_ver,anexo,e_mail,id_foto,id_unidad,unidad,empresa FROM view_ges_InfoRut  WHERE (rut = ")).append(paramRut).append(")")));
            consul.exec(sql);
            if(consul.next())
            {
            ...
            Consulta consulRuts = new Consulta(conexion);
            if(req.getParameter("unirel") != null && req.getParameter("rel_emp") != null && req.getParameter("rel_uni") != null && req.getParameter("rel_tipo") != null && !"".equals(req.getParameter("rel_emp")) && !"".equals(req.getParameter("rel_uni")) && !"".equals(req.getParameter("rel_tipo")))
            {
                tieneUnidRel = true;
                unidRel = new VerUnidad(req.getParameter("rel_emp"), req.getParameter("rel_uni"), req.getParameter("rel_tipo"));
            ...
            consul.close();
            super.retTemplate(resp,"Gestion/Usuarios/MantenerUsuario.htm",modelRoot);
        } else
        {
            mensaje.devolverPaginaSinSesion(resp, "", "Tiempo de Sesi\363n expirado...");
        }
