...
public class creaArbol
{
    ...
    public Nodo getNodo(String empresa, String descripcion)
    {
        ...
        sql = String.valueOf(String.valueOf((new StringBuilder("SELECT DISTINCT empresa, hijo, descr, padre FROM VIEW_GES_arbol WHERE (padre = '0') AND empresa = '")).append(IDEmpresa).append("' ORDER BY descr")));
        ...
        for(; hijos.next(); NE.add(nodoPadre))
        {
            ...
            sql = String.valueOf(String.valueOf((new StringBuilder("Select DISTINCT empresa, hijo, descr, padre from VIEW_GES_arbol where padre <> '0' and padre='")).append(id).append("' and empresa='").append(IDEmpresa).append("' ORDER BY descr")));
