$(document).ready(function(){
	        ...
	        $.get('../servlet/Tool?xml/noticias.xml', function(xml) {
				...
				$(".submit_login").keypress(function( event ) {
					if(event.which == 13) {
						loginIngresoMismaVentana(document.form1, true)	
					}
				});
...
function  boton_numerico(digito) {
	...
	function loginIngreso(f, omitValidacion ) {
		var valor = document.getElementById("param").value;
		if (!omitValidacion && !checkDigitoV(f.rut.value, f.dig.value)) {
			alert('Rut no v&aacute;lido...');
			f.rut.focus();
		}
		else {
	...
	function loginIngresoMismaVentana(f, omitValidacion) {
		if (!omitValidacion && !checkDigitoV(f.rut.value, f.dig.value)) {
			alert('Rut no v&aacute;lido...');
			f.rut.focus();
		}
	...
	function fnRecuperarClave(rut, dv) {
		var rut = rut+'-'+dv; 
		if (fnValidaRut(rut)) {
			loadJSONforMain('../servlet/EjeCoreI?claseweb=cl.eje.mail.modulos.RecuperaClave',
					'GET',{"rut":rut, "accion":"recovery", "thing":"pass"},'json',
				function(data){
					
					if(data.estado == "false") {
						
						var getcorreo = data.if_correo_not_valid_then_getcorreo;
						var getcell = data.if_correo_not_valid_then_getcellphone;
						...
						if(getcorreo == "true" && getcell == "true") {
							jPrompt('Ingrese su correo electr\u00F3nico   . A este correo lo contactaremos', correo, 'Contacto', function(r) {
								if(r != null && r != '') {
									var correo = r;
									jPrompt('Ingrese alg\u00FAn n\u00FAmero de contacto, esta ser\u00E1 la segunda opci\u00F3n en caso que no funcione el correo', "09 9 1234 5678", 'Contacto', function(r) {
										if(r != null && r != '') {
											var fono = r;
											...
											loadJSONforMain('../servlet/EjeCoreI?claseweb=cl.eje.mail.modulos.RecuperaClave',
													...
													function(data){
														 jAlert(data.msg,"Contacto");
													});
										}
										else {
											jAlert("No se ha podido enviar el correo", "Error COD:1");
										}
									});
								}
								else {
						...
						else if(getcorreo == "true") {
							jPrompt('Ingrese su correo electr\u00F3nico   . A este correo lo contactaremos', correo , 'Contacto', function(inputCorreo) {
								if(inputCorreo != null && inputCorreo != '') {
								 	loadJSONforMain('../servlet/EjeCoreI?claseweb=cl.eje.mail.modulos.RecuperaClave',
											'GET',{"rut":rut,"correo":inputCorreo,"accion":"help", "thing":"pass"},'json',
						...
						else if(getcell == "true") {
							jPrompt('Ingrese alg\u00FAn n\u00FAmero de contacto, esta ser\u00E1 la segunda opci\u00F3n en caso que no funcione el correo', "09 9 1234 5678", 'Contacto', function(r) {
								if(r != null && r != '') {
									var fono = r;
									
									loadJSONforMain('../servlet/EjeCoreI?claseweb=cl.eje.mail.modulos.RecuperaClave',
											'GET',{"rut":rut,"fono":fono ,"accion":"help", "thing":"pass"},'json',
							...
							jAlert("No se ha podido enviar el correo", "Error COD:1001");
						}
					}else{
						$("#msgcorreo").text(data.email);
						$("#messagedialog").dialog( "open" );				
						$("#dialog-rc").dialog( "close" );
...
function Valida_rut(texto) {
	...
	if ( largo < 2 )	
	{		
		jAlert("El rut ingresado es inv\u00E1lido","Atenci\u00F3n");
	...
	var rut = invertido.replace('.','').replace('.','');
	var dv = invertido.substring(invertido.indexOf('-')+1);
...
function fnValidaRut(texto) {
	...
	window.document.form1.dig.value = dv;
	
	if (fnRevisarDigitoVerificador(texto)){
		return true;	
	}else{
		return false;
	}
...
function fnRevisarDigitoVerificador(crut) {
